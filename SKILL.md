...
7. **Record the check** for idempotency (see below), even on a no-move pass.
...
- **Recheck window.** Skip anyone checked within `<RECHECK_DAYS>`. Track the check date. Preferred: a `linkedin_checked` date attribute on people (customer adds it; the MCP cannot create attributes). If that field is absent, fall back to parsing the date prefix of the latest `last_job_change` entry, and to Champion Moves list membership.
